...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    await conn.sendGroupV4Invite(m.chat, jid, invite_code, invite_code_exp, false, 'Hey!! Hola, me presento, soy SKY - Bot, una persona del grupo utilizo el comando para añadirte al grupo, pero no pude agregarte, asi que te mando la invitacion para que te agregues, te esperamos!!', jpegThumbnail ? {
      jpegThumbnail
    } : {})
  }
}
